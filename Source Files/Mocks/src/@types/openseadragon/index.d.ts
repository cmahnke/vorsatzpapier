declare namespace OpenSeadragon {
  ...
  function delegate(object: object, method: (...args: any[]) => void): (object: any, ...args: any[]) => void;
  ...
  function normalizeEventListenerOptions(options: boolean | { capture?: boolean; passive?: boolean; once?: boolean }): string;
  ...
  class CanvasDrawer extends DrawerBase {
    ...
    get canvas(): HTMLCanvasElement;
    blendSketch(options: { opacity: number; scale: number; translate?: Point; compositeOperation?: string; bounds?: Rect }): void;
  }
  ...
  class Spring {
    ...
    start: TSpringObj;
    target: TSpringObj;
    constructor(options: { springStiffness: number; animationTime: number; initial?: number; exponential?: boolean });
  ...
  class TileCache {
    ...
    cacheTile(options: { tile: Tile; image: HTMLImageElement; tiledImage: TiledImage; cutoff?: number }): void;
  ...
  class TiledImage extends EventSource<TiledImageEventMap> {
    ...
    // Patched by patchOpenSeadragon
    /** @internal Patched by patchOpenSeadragon - computes the rotation point in viewport coordinates */
    _getRotationPoint(current: boolean): Point;
    /** Patched by patchOpenSeadragon - sets a custom rotation point in image coordinates */
    setRotationPoint(rotationPoint: Point): void;
    /** Patched by patchOpenSeadragon - returns the position of the tiled image in viewport coordinates */
    getPosition(current?: boolean): Point;
    /** Patched by patchOpenSeadragon - exposes the internal _clip property */
    clip: Rect;
    _clip?: Rect;
  ...
  class TileSource extends EventSource<TileSourceEventMap> {
    ...
    /**
     * Patched by patchOpenSeadragon - returns a Point representing the tile
     * x/y grid coordinates at the given level containing the given point,
     * rather than the original Tile object.
     */
  ...
  class Viewport {
    ...
    /**
     * Patched by patchOpenSeadragon - sets the content bounds and raises
     * the "reset-size" event on the viewer. Skips update if any bound
     * dimension is NaN.
     * @internal
     */
    _setContentBounds(bounds: Rect, contentFactor: number): void;
  ...
  interface PreProcessMouseTrackerEvent extends MouseTrackerEvent<
    KeyboardEvent | FocusEvent | WheelEvent | SimulatedWheelEvent | MouseEvent | TouchEvent | PointerEvent
  > {
